...
class Forecast_Nowcast_Allign:
    ...
    def _identify_prefixes_ensemble(self, dataset: pd.DataFrame) -> set[str]:
        ...
        """
        prefixes = set(re.match(r"(.*)_\d+$", c).group(1) for c in dataset.columns if re.match(r".*_\d+$", c))
    ...
    def _min_max_date_dataset(self, column: str, dataset: pd.DataFrame):
        ...
        dataset = dataset[dataset[column].between(min_date, max_date)].copy()
    ...
    def _add_windparks_into_regions(self, dataset: pd.DataFrame, wcol: str, keys: list[str], wmean_cols: list[str]):
        ...
        region_df = num_sum.div(w_sum, axis=0).reset_index().merge(w_sum.reset_index(), on=keys, how="left")
    ...
    def save_merged_dataset(
        ...
        path: str = "src/processed_data/pipeline_data/met_forecast_nowcast_merged.parquet",
    ):
    ...
    def merge_datasets(self):
        ...
        self.nowcast[self.config.windpark_col] = self.nowcast[self.config.windpark_col].apply(
            self._clean_names_windparks
        )
        ...
        self.windpark[self.config.region_col] = (
            self.windpark[self.config.bidding_area_col].str.replace("ELSPOT ", "", regex=False).str.strip()
        )
        self.windpark[self.config.prod_start_new_col] = pd.to_datetime(
            self.windpark[self.config.prod_start_new_col], errors="coerce"
        )
        ...
        meta_merged = merged.merge(
            self.windpark[
                [
                    self.config.substation_name_col,
                    self.config.region_col,
                    self.config.operating_power_max_col,
                    self.config.prod_start_new_col,
                ]
            ],
        ...
        prec = (
            meta_merged.assign(precipitation_amount=pd.to_numeric(meta_merged["precipitation_amount"], errors="coerce"))
            .groupby(keys)["precipitation_amount"]
            .sum()
            .reset_index()
        )
        ...
        region_df = self._add_circular_mean(
            region_df, meta_merged, "wind_direction_10m", "wind_direction_10m", wcol, keys
        )
        ...
        self.power[self.config.region_col] = self.power[self.config.region_col].str.replace("ELSPOT ", "", regex=False)
